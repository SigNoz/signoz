...
describe('AIObservability (integration)', () => {
	beforeEach(() => {
	...
	it('renders the overview panel and the tab bar on the overview route', () => {
		render(<AIObservability />, undefined, {
			initialRoute: ROUTES.AI_OBSERVABILITY_OVERVIEW,
		});

		expect(screen.getByTestId('ai-observability-tabs')).toBeInTheDocument();
		expect(screen.getByTestId('ai-observability-overview')).toBeInTheDocument();
	...
	it('renders the model-pricing page on the configuration route', async () => {
		...
		await waitFor(() =>
			expect(
				screen.getByTestId('ai-observability-model-pricing-page'),
			).toBeInTheDocument(),
		);
	});
});
