...
function DynamicVariableSelection({
	...
	const { isLoading, refetch } = useQuery(
		...
		],
		{
			...
			onSuccess: (data) => {
				setOptionsData(data.data?.normalizedValues || []);
				setIsComplete(data.data?.complete || false);
				setFilteredOptionsData(data.data?.normalizedValues || []);
				setRelatedValues(data.data?.relatedValues || []);
				setOriginalRelatedValues(data.data?.relatedValues || []);
			},
			onError: (error: any) => {
				if (error) {
