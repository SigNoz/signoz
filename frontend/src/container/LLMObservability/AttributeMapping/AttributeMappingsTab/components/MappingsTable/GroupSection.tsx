 ...
 * and the real controls here (toggle, kebab) carry their own labels.
 */
...
import { DraftGroup, Mapper } from '../../../types';
...
// Fade shared by the non-row states (skeleton / error / empty) so they reveal
// in step with the mapper rows on expand and fade back out on collapse. Wrapped
// in AnimatePresence below so the exit runs before unmount, making expand/
// collapse read as one accordion rather than a snap.
...
function GroupSection({
	...
	// the group has been opened (or already carries loaded mappers). Hiding it
	...
	// · status) so the lazy per-group load mirrors real rows rather than flat bars.
	...
	// run the per-row enter/exit fade on expand and collapse.
	...
	if (expanded && isErrorMappers) {
		...
		expandedRows = skeletonRows;
	} else if (expanded && mapperCount === 0) {
		expandedRows = [emptyRow];
	} else if (expanded) {
		expandedRows = mapperRows;
	}
