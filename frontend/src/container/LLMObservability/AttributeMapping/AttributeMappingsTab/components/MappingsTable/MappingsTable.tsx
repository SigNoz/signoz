...
import MapperFormDrawer from 'container/LLMObservability/AttributeMapping/components/MapperFormDrawer';
import { useMapperFormDrawer } from 'container/LLMObservability/AttributeMapping/useMapperFormDrawer';
import {
	DraftGroup,
	DraftMapper,
...
// its mappers query disables. The mapper drawer lives here once — a target
// group is tracked so a single instance serves add/edit for whichever group
// triggered it.
function MappingsTable({
	...
	const items: CollapseProps['items'] = store.groups.map((group) => ({
		...
		label: (
			<GroupHeader
				group={group}
				expanded={expandedGroups.includes(group.localId)}
			/>
		),
		extra: (
			<GroupHeaderActions
