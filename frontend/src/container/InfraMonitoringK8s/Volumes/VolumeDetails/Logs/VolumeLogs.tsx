...
import { getVolumeLogsQueryPayload } from './constants';
...
function PodLogs({
	...
	useEffect(() => {
		const newRestFilters = filters.items.filter(
			(item) =>
				item.key?.key !== 'id' &&
				![QUERY_KEYS.K8S_VOLUME_NAME].includes(item.key?.key ?? ''),
		);
	...
	const queryPayload = useMemo(() => {
		const basePayload = getVolumeLogsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	const { data, isLoading, isFetching, isError } = useQuery({
		queryKey: ['volumeLogs', timeRange.startTime, timeRange.endTime, filters],
	...
	const renderContent = useMemo(
		() => (
			<Card bordered={false} className="volume-logs-list-card">
				<OverlayScrollbar isVirtuoso>
					<Virtuoso
						className="volume-logs-virtuoso"
						key="volume-logs-virtuoso"
						data={logs}
	...
	return (
		<div className="volume-logs">
			{isLoading && <LogsLoading />}
			{!isLoading && !isError && logs.length === 0 && <NoLogsContainer />}
			{isError && !isLoading && <LogsError />}
			{!isLoading && !isError && logs.length > 0 && (
				<div className="volume-logs-list-container">{renderContent}</div>
			)}
		</div>
	);
}
