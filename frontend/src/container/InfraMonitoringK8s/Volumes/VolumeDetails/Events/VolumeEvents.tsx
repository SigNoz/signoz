...
import './VolumeEvents.styles.scss';
...
import { getVolumesEventsQueryPayload } from './constants';
...
export default function Events({
	...
	handleTimeChange,
	selectedInterval,
}: IVolumeEventsProps): JSX.Element {
	const { currentQuery } = useQueryBuilder();

	const [formattedVolumeEvents, setFormattedVolumeEvents] = useState<
		EventDataType[]
	>([]);
	...
	const queryPayload = useMemo(() => {
		const basePayload = getVolumesEventsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	const { data: eventsData, isLoading, isFetching, isError } = useQuery({
		queryKey: ['volumeEvents', timeRange.startTime, timeRange.endTime, filters],
	...
	useEffect(() => {
		if (eventsData?.payload?.data?.newResult?.data?.result) {
			...
			setFormattedVolumeEvents(formattedData);
	...
	const handlePrev = (): void => {
		if (!formattedVolumeEvents.length) return;

		setPage(page - 1);

		const firstEvent = formattedVolumeEvents[0];
	...
	const handleNext = (): void => {
		...
		setPage(page + 1);
		const lastEvent = formattedVolumeEvents[formattedVolumeEvents.length - 1];
	...
	return (
		<div className="volume-events-container">
			<div className="volume-events-header">
			...
			{!isLoading && !isError && formattedVolumeEvents.length === 0 && (
				<NoEventsContainer />
			)}
			...
			{!isLoading && !isError && formattedVolumeEvents.length > 0 && (
				<div className="volume-events-list-container">
					<div className="volume-events-list-card">
						<Table<EventDataType>
							...
							dataSource={formattedVolumeEvents}
							pagination={false}
			...
			{!isError && formattedVolumeEvents.length > 0 && (
				<div className="volume-events-footer">
					<Button
						className="volume-events-footer-button periscope-btn ghost"
						type="link"
						onClick={handlePrev}
