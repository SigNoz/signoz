...
import './StatefulSetLogs.styles.scss';
...
import { getStatefulSetLogsQueryPayload } from './constants';
...
function PodLogs({
	...
	const queryPayload = useMemo(() => {
		const basePayload = getStatefulSetLogsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	const renderContent = useMemo(
		() => (
			<Card bordered={false} className="statefulSet-logs-list-card">
				<OverlayScrollbar isVirtuoso>
					<Virtuoso
						className="statefulSet-logs-virtuoso"
						key="statefulSet-logs-virtuoso"
						data={logs}
	...
	return (
		<div className="statefulSet-logs">
			{isLoading && <LogsLoading />}
			{!isLoading && !isError && logs.length === 0 && <NoLogsContainer />}
			{isError && !isLoading && <LogsError />}
			{!isLoading && !isError && logs.length > 0 && (
				<div className="statefulSet-logs-list-container">{renderContent}</div>
			)}
		</div>
	);
}
