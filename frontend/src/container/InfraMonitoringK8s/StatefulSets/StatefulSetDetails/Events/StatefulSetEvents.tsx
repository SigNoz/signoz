...
import './StatefulSetEvents.styles.scss';
...
import { getStatefulSetsEventsQueryPayload } from './constants';
...
interface IStatefulSetEventsProps {
	timeRange: {
		startTime: number;
		endTime: number;
	};
...
export default function Events({
	...
	handleTimeChange,
	selectedInterval,
}: IStatefulSetEventsProps): JSX.Element {
	const { currentQuery } = useQueryBuilder();

	const [formattedStatefulSetEvents, setFormattedStatefulSetEvents] = useState<
		EventDataType[]
	>([]);
	...
	const queryPayload = useMemo(() => {
		const basePayload = getStatefulSetsEventsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	useEffect(() => {
		if (eventsData?.payload?.data?.newResult?.data?.result) {
			...
			setFormattedStatefulSetEvents(formattedData);
	...
	const handlePrev = (): void => {
		if (!formattedStatefulSetEvents.length) return;

		setPage(page - 1);

		const firstEvent = formattedStatefulSetEvents[0];
	...
	const handleNext = (): void => {
		...
		setPage(page + 1);
		const lastEvent =
			formattedStatefulSetEvents[formattedStatefulSetEvents.length - 1];
	...
	return (
		<div className="statefulSet-events-container">
			<div className="statefulSet-events-header">
			...
			{!isLoading && !isError && formattedStatefulSetEvents.length === 0 && (
				<NoEventsContainer />
			)}
			...
			{!isLoading && !isError && formattedStatefulSetEvents.length > 0 && (
				<div className="statefulSet-events-list-container">
					<div className="statefulSet-events-list-card">
						<Table<EventDataType>
							...
							dataSource={formattedStatefulSetEvents}
							pagination={false}
			...
			{!isError && formattedStatefulSetEvents.length > 0 && (
				<div className="statefulSet-events-footer">
					<Button
						className="statefulSet-events-footer-button periscope-btn ghost"
						type="link"
						onClick={handlePrev}
