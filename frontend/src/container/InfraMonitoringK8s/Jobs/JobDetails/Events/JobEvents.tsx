...
import { getJobsEventsQueryPayload } from './constants';
...
export default function Events({
	...
	handleTimeChange,
	selectedInterval,
}: IJobEventsProps): JSX.Element {
	const { currentQuery } = useQueryBuilder();

	const [formattedJobEvents, setFormattedJobEvents] = useState<EventDataType[]>(
		[],
	);
	...
	const queryPayload = useMemo(() => {
		const basePayload = getJobsEventsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	const { data: eventsData, isLoading, isFetching, isError } = useQuery({
		queryKey: ['jobEvents', timeRange.startTime, timeRange.endTime, filters],
	...
	useEffect(() => {
		if (eventsData?.payload?.data?.newResult?.data?.result) {
			...
			setFormattedJobEvents(formattedData);
	...
	const handlePrev = (): void => {
		if (!formattedJobEvents.length) return;

		setPage(page - 1);

		const firstEvent = formattedJobEvents[0];
	...
	const handleNext = (): void => {
		...
		setPage(page + 1);
		const lastEvent = formattedJobEvents[formattedJobEvents.length - 1];
	...
	return (
		<div className="job-events-container">
			<div className="job-events-header">
			...
			{!isLoading && !isError && formattedJobEvents.length === 0 && (
				<NoEventsContainer />
			)}
			...
			{!isLoading && !isError && formattedJobEvents.length > 0 && (
				<div className="job-events-list-container">
					<div className="job-events-list-card">
						<Table<EventDataType>
			...
			{!isError && formattedJobEvents.length > 0 && (
				<div className="job-events-footer">
					<Button
						className="job-events-footer-button periscope-btn ghost"
						type="link"
						onClick={handlePrev}
