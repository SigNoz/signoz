...
import { getJobLogsQueryPayload } from './constants';
...
function PodLogs({
	...
	const queryPayload = useMemo(() => {
		const basePayload = getJobLogsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	const { data, isLoading, isFetching, isError } = useQuery({
		queryKey: ['jobLogs', timeRange.startTime, timeRange.endTime, filters],
	...
	const renderContent = useMemo(
		() => (
			<Card bordered={false} className="job-logs-list-card">
				<OverlayScrollbar isVirtuoso>
					<Virtuoso
						className="job-logs-virtuoso"
						key="job-logs-virtuoso"
						data={logs}
	...
	return (
		<div className="job-logs">
			...
			{!isLoading && !isError && logs.length > 0 && (
				<div className="job-logs-list-container">{renderContent}</div>
			)}
		</div>
	);
}
