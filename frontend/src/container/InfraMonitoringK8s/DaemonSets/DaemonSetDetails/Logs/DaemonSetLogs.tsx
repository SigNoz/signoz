...
import './DaemonSetLogs.styles.scss';
...
import { getDaemonSetLogsQueryPayload } from './constants';
...
function PodLogs({
	...
	const queryPayload = useMemo(() => {
		const basePayload = getDaemonSetLogsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	const { data, isLoading, isFetching, isError } = useQuery({
		queryKey: ['daemonSetLogs', timeRange.startTime, timeRange.endTime, filters],
	...
	const renderContent = useMemo(
		() => (
			<Card bordered={false} className="daemonSet-logs-list-card">
				<OverlayScrollbar isVirtuoso>
					<Virtuoso
						className="daemonSet-logs-virtuoso"
						key="daemonSet-logs-virtuoso"
						data={logs}
	...
	return (
		<div className="daemonSet-logs">
			{isLoading && <LogsLoading />}
			{!isLoading && !isError && logs.length === 0 && <NoLogsContainer />}
			{isError && !isLoading && <LogsError />}
			{!isLoading && !isError && logs.length > 0 && (
				<div className="daemonSet-logs-list-container">{renderContent}</div>
			)}
		</div>
	);
}
