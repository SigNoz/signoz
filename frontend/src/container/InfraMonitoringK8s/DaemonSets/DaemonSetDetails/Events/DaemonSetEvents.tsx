...
import './DaemonSetEvents.styles.scss';
...
import { getDaemonSetsEventsQueryPayload } from './constants';
...
interface IDaemonSetEventsProps {
	timeRange: {
		startTime: number;
		endTime: number;
	};
...
export default function Events({
	...
	handleTimeChange,
	selectedInterval,
}: IDaemonSetEventsProps): JSX.Element {
	const { currentQuery } = useQueryBuilder();

	const [formattedDaemonSetEvents, setFormattedDaemonSetEvents] = useState<
		EventDataType[]
	>([]);
	...
	const queryPayload = useMemo(() => {
		const basePayload = getDaemonSetsEventsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	useEffect(() => {
		if (eventsData?.payload?.data?.newResult?.data?.result) {
			...
			setFormattedDaemonSetEvents(formattedData);
	...
	const handlePrev = (): void => {
		if (!formattedDaemonSetEvents.length) return;

		setPage(page - 1);

		const firstEvent = formattedDaemonSetEvents[0];
	...
	const handleNext = (): void => {
		...
		setPage(page + 1);
		const lastEvent =
			formattedDaemonSetEvents[formattedDaemonSetEvents.length - 1];
	...
	return (
		<div className="daemonSet-events-container">
			<div className="daemonSet-events-header">
			...
			{!isLoading && !isError && formattedDaemonSetEvents.length === 0 && (
				<NoEventsContainer />
			)}
			...
			{!isLoading && !isError && formattedDaemonSetEvents.length > 0 && (
				<div className="daemonSet-events-list-container">
					<div className="daemonSet-events-list-card">
						<Table<EventDataType>
							...
							dataSource={formattedDaemonSetEvents}
							pagination={false}
			...
			{!isError && formattedDaemonSetEvents.length > 0 && (
				<div className="daemonSet-events-footer">
					<Button
						className="daemonSet-events-footer-button periscope-btn ghost"
						type="link"
						onClick={handlePrev}
