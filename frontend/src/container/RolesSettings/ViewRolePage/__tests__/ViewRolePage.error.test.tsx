...
describe('ViewRolePage - Error State', () => {
	...
	it('shows Back to roles button on error state', () => {
		jest.spyOn(roleApi, 'useGetRole').mockReturnValue({
			data: undefined,
			isLoading: false,
			isError: true,
		...
		expect(cancelButton).toHaveTextContent('Back to roles');
	});

	it('navigates to roles list when Back to roles clicked on error state', async () => {
