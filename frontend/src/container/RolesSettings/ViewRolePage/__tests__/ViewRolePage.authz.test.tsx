...
import { BrandedPermission, UseAuthZResult } from 'hooks/useAuthZ/types';
...
function mockUseAuthZGrantReadDeleteDenied(
	...
	return {
		...
		permissions: Object.fromEntries(
			permissions.map((p) => [
				p,
				{ isGranted: p.startsWith('read') || p.startsWith('update') },
			]),
...
function mockUseAuthZGrantReadUpdateDenied(
	permissions: BrandedPermission[],
): UseAuthZResult {
	return {
		isLoading: false,
		...
		permissions: Object.fromEntries(
			permissions.map((p) => [
				p,
				{ isGranted: p.startsWith('read') || p.startsWith('delete') },
			]),
		) as UseAuthZResult['permissions'],
		refetchPermissions: jest.fn(),
	};
