...
// With use_json_body or json_body_dual_ingestion the collector normalizes every body
// into a map before user operators run, so a parser pointed at `body` gets a map it
// cannot read and silently extracts nothing. The log text lives at body.message.
