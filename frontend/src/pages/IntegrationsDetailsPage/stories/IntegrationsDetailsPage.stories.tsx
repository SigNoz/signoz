...
const pageStory = storyMocks(integrationDetailMocks, { layout: 'app' });
...
const clickTab = async (
	...
	await userEvent.click(await canvas.findByText(label, undefined, untilLoaded));
	// The panels swap a render after the click, so the tab's own selected state
	// is what says the story has reached the tab it names.
	await canvas.findByRole('tab', { name: label, selected: true }, untilLoaded);
};
