...
import { generateTicks } from './utils/timeAxisUtils';
...
function TimeAxis({ timeRange, width, timezone }: TimeAxisProps): JSX.Element {
	...
	return (
		<div style={containerStyle} data-testid="state-timeline-time-axis">
			{ticks.map((tick) => {
				const leftPx = tick.position * width;
				return (
					<div key={tick.timestamp}>
						<div style={{ ...tickLineStyle, left: `${leftPx}px` }} />
						<div style={{ ...tickLabelStyle, left: `${leftPx}px` }}>{tick.label}</div>
					</div>
				);
			})}
		</div>
