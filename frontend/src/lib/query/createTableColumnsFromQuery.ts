...
const addOperatorFormulaColumns = (
	...
	if (queryType === EQueryType.CLICKHOUSE) {
		...
		if (currentQueryData.legend) {
			// TODO (vikrantgupta25) do not add title when only one present
			if (title) {
				operatorLabel = `${title}-${currentQueryData.legend}`;
			} else {
				operatorLabel = currentQueryData.legend;
			}
		} else if (title) {
		...
		const operatorColumn: DynamicColumn = {
			query,
			field: title || currentQueryData.name,
			dataIndex: title || currentQueryData.name,
...
const fillRestAggregationData = (
	...
	const nextQueryData =
		queryTableData.find((q) => q.title === column.field) || null;
...
const fillDataFromSeries = (
	...
	const { series, queryName, title } = currentQuery;
	const isEqualQuery = isEqualQueriesByLabel(
	...
	series.forEach((seria) => {
		...
		columns.forEach((column) => {
			...
			if (column.type !== 'field' && column.field !== title) {
...
/**
 *
 * @param tableData the sorted table data generated by sorting on label array keys and values
 * @returns new table data with multiple aggregation series inside same query to multiple queries with same name
 */
...
const splitSeriesOnTitle = (tableData: QueryDataV3[]): QueryDataV3[] => {
	...
	tableData.forEach((query) => {
		...
		if (Object.keys(titleMap).length === 1) {
			...
			updatedQuery.title = Object.keys(titleMap)[0] || query.queryName;
			splitSeriesData.push(updatedQuery);
		} else {
			Object.keys(titleMap).forEach((title) => {
				if (title !== '') {
					const updatedSeries = cloneDeep(query);
					updatedSeries.series = titleMap[title];
					updatedSeries.title = title;
