...
└── e2e/
    ...
    ├── bootstrap/
    │   ├── setup.py           # test_setup / test_teardown — the long-running form
    │   └── run.py             # test_e2e — one-command form (setup → yarn test)
    ├── fixtures/
...
Or, one-command (pytest brings up the stack, shells out to `yarn test`, tears down on failure):
...
Loading order in `playwright.config.ts`: `.env` first (user-provided, staging), then `.env.local` with `override: true` (bootstrap-generated, local mode). Subprocess-injected env (e.g. `run.py` shelling out to `yarn test`) still wins because dotenv doesn't touch vars already set in `process.env`.
