...
class TestTTLMethods:
    ...
    def test_set_ttl_traces_success(self):
    ...
    def test_set_custom_retention_ttl_basic(self):
    ...
    def test_get_custom_retention_ttl(self, insert_logs):
        ...
        set_response = self._make_ttl_request(
            "/api/v2/settings/ttl", json_body=set_payload
        )
        ...
        get_response = self._make_ttl_request(
            "/api/v2/settings/ttl", query_params={"type": "logs"}, method=requests.get
        )
        ...
        assert (
            response_data["ttl_conditions"][0]["conditions"][0]["key"] == "service_name"
        )
    ...
    def test_get_ttl_traces_success(self):
        ...
        set_response = self._make_ttl_request(
            "/api/v1/settings/ttl", query_params=set_payload
        )
        assert set_response.status_code == HTTPStatus.OK
        ...
        get_response = self._make_ttl_request(
            "/api/v1/settings/ttl", query_params={"type": "traces"}, method=requests.get
        )
    ...
    def test_large_ttl_conditions_list(self, insert_logs):
        ...
        for i in range(10):  # Test with 10 conditions
            conditions.append(
                {
                    "conditions": [
                        {"key": "service_name", "values": [f"service-{i}"]}
                    ],
                    "ttlDays": 30 + (i * 10),
                }
            )
