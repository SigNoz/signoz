...
def test_zeus_activate_license(
    signoz: types.SigNoz,
    make_http_mocks: Callable[[types.TestContainerDocker, list[Mapping]], None],
    get_token: Callable[[str, str], str],
) -> None:
        ...
        [
            Mapping(
                request=MappingRequest(
                    method=HttpMethods.GET,
                    url="/v2/licenses/me",
                    headers={"X-Signoz-Cloud-Api-Key": {WireMockMatchers.EQUAL_TO: "secret-key-zeus"}},
                ),
                response=MappingResponse(
                    status=200,
                    json_body={
                        "status": "success",
                        "data": {
                            "id": "0196360e-90cd-7a74-8313-1aa815ce2a68",
                            "key": "secret-key-zeus",
                            "valid_from": 1732146924,
                            "valid_until": -1,
                            "status": "VALID",
                            "state": "EVALUATING",
                            "plan": {
    ...
    response = requests.post(
        url=signoz.self.host_configs["8080"].get("/api/v2/zeus/licenses"),
        json={"key": "secret-key-zeus"},
        headers={"Authorization": "Bearer " + access_token},
        timeout=5,
    )
...
def test_zeus_get_active_license(
    signoz: types.SigNoz,
    get_token: Callable[[str, str], str],
) -> None:
    access_token = get_token("admin@integration.test", "password123Z$")
    ...
    assert response.status_code == http.HTTPStatus.OK
    assert response.json()["data"]["id"] == "0196360e-90cd-7a74-8313-1aa815ce2a68"
    assert response.json()["data"]["key"] == "secret-key-zeus"
    assert response.json()["data"]["valid_from"] == 1732146924
    assert response.json()["data"]["status"] == "VALID"
    assert response.json()["data"]["state"] == "EVALUATING"
    assert response.json()["data"]["platform"] == "CLOUD"
    assert response.json()["data"]["plan"]["name"] == "ENTERPRISE"


def test_zeus_refresh_license(
    signoz: types.SigNoz,
    make_http_mocks: Callable[[types.TestContainerDocker, list[Mapping]], None],
    get_token: Callable[[str, str], str],
) -> None:
    ...
    assert response.status_code == http.HTTPStatus.OK
    assert response.json()["data"]["valid_from"] == 1732146926
