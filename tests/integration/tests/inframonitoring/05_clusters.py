...
def test_clusters_filter_and(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
) -> None:
    """AND of two attribute clauses returns only the matching clusters."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "cloud.provider = 'gcp' AND env = 'prod'"},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
    assert {r["clusterName"] for r in data["records"]} == {"web-gcp-prod", "api-gcp-prod"}
    assert data["total"] == 2


def test_clusters_filter_in(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
) -> None:
    """IN (...) returns exactly the listed clusters."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
            get_testdata_file_path("inframonitoring/clusters_filter_dataset.jsonl"),
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "k8s.cluster.name IN ('web-gcp-prod', 'api-aws-dev')"},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
    assert {r["clusterName"] for r in data["records"]} == {"web-gcp-prod", "api-aws-dev"}
    assert data["total"] == 2


def test_clusters_filter_not_in(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    NOT IN on a non-partition label (e.g. cloud.provider) is unreliable — likely
    a QB v5 label-extraction gap; covered indirectly via the and_not_in combo."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
            get_testdata_file_path("inframonitoring/clusters_filter_dataset.jsonl"),
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": ("k8s.cluster.name NOT IN ('web-gcp-prod', 'web-gcp-dev', 'api-gcp-prod', 'api-gcp-dev')")},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
    assert {r["clusterName"] for r in data["records"]} == {
        "web-aws-prod",
        "web-aws-dev",
        "api-aws-prod",
        "api-aws-dev",
...
def test_clusters_filter_contains(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "k8s.cluster.name CONTAINS 'web'"},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
...
def test_clusters_filter_combos(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
...
def test_clusters_filter_bad_attr_name(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "k8s.cluster.namee = 'web-gcp-prod'"},
        },
    )
    ...
    assert any("k8s.cluster.namee" in e["message"] for e in body["error"]["errors"]), f"bad attr name not surfaced: {body['error']['errors']!r}"


@pytest.mark.parametrize(
    "expression",
    [
        pytest.param("k8s.cluster.name =", id="trailing_op"),
        pytest.param("(k8s.cluster.name = 'web-gcp-prod'", id="unclosed_paren"),
    ],
)
def test_clusters_filter_bad_grammar(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
...
def test_clusters_total_invariant_across_orderby(
    ...
    """Total stays K across all 4 orderBy metric columns x 2 directions = 8 calls."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
