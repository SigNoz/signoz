...
def test_statefulsets_happy_path(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
) -> None:
    """2 statefulsets x 2 pods/SS, all Running; assert response shape + counts."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
            get_testdata_file_path("inframonitoring/statefulsets_happy_path.jsonl"),
    ...
    assert {r["statefulSetName"] for r in data["records"]} == {"happy-ss-1", "happy-ss-2"}
...
def test_statefulsets_value_accuracy(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
) -> None:
    """Exact per-SS metric values + replica counts + phase counts.
...
def test_statefulsets_filter_and(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
) -> None:
    """AND of two attribute clauses returns only the matching statefulsets."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
    ...
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
    assert {r["statefulSetName"] for r in data["records"]} == {"web-a-prod", "api-a-prod"}
    assert data["total"] == 2


def test_statefulsets_filter_in(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
) -> None:
    """IN (...) returns exactly the listed statefulsets."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
            get_testdata_file_path("inframonitoring/statefulsets_filter_dataset.jsonl"),
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "k8s.statefulset.name IN ('web-a-prod', 'api-b-dev')"},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
    assert {r["statefulSetName"] for r in data["records"]} == {"web-a-prod", "api-b-dev"}
    assert data["total"] == 2


def test_statefulsets_filter_not_in(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "k8s.statefulset.name NOT IN ('web-a-prod', 'web-a-dev', 'api-a-prod', 'api-a-dev')"},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
    assert {r["statefulSetName"] for r in data["records"]} == {
        "web-b-prod",
        "web-b-dev",
        "api-b-prod",
        "api-b-dev",
...
def test_statefulsets_filter_contains(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "k8s.statefulset.name CONTAINS 'web'"},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
...
def test_statefulsets_filter_combos(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
...
def test_statefulsets_filter_bad_attr_name(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "filter": {"expression": "k8s.statefulset.namee = 'web-a-prod'"},
        },
    )
    ...
    assert any("k8s.statefulset.namee" in e["message"] for e in body["error"]["errors"]), f"bad attr name not surfaced: {body['error']['errors']!r}"


@pytest.mark.parametrize(
    "expression",
    [
        pytest.param("k8s.statefulset.name =", id="trailing_op"),
        pytest.param("(k8s.statefulset.name = 'web-a-prod'", id="unclosed_paren"),
    ],
)
def test_statefulsets_filter_bad_grammar(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
...
def test_statefulsets_pagination_sync(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
...
def test_statefulsets_offset_beyond_total(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
...
def test_statefulsets_total_invariant_across_orderby(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    # orderBy keys per statefulsets_constants.go:5-14.
    for column in (
        "cpu",
        "cpu_request",
        "cpu_limit",
...
def test_statefulsets_orderby_correctness(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    Covers each entry in orderByToStatefulSetsQueryNames (statefulsets_constants.go:47-56)."""
    now = datetime.now(tz=UTC).replace(microsecond=0)
    insert_metrics(
        Metrics.load_from_file(
            get_testdata_file_path("inframonitoring/statefulsets_orderby.jsonl"),
...
@pytest.mark.parametrize("direction", ["asc", "desc"])
def test_statefulsets_orderby_by_statefulset_name(
    signoz: types.SigNoz,
    create_user_admin: None,  # pylint: disable=unused-argument
    get_token,
    insert_metrics,
    ...
    """orderBy=k8s.statefulset.name with empty groupBy returns SS sorted
    ...
    response = _post(
        ...
        token,
        {
            ...
            "end": int(now.timestamp() * 1000),
            "limit": 50,
            "orderBy": {"key": {"name": "k8s.statefulset.name"}, "direction": direction},
            "filter": {"expression": "k8s.statefulset.name CONTAINS 'order-'"},
        },
    )
    assert response.status_code == HTTPStatus.OK, response.text
    data = response.json()["data"]
    names = [r["statefulSetName"] for r in data["records"]]
    expected = sorted(names, reverse=(direction == "desc"))
    assert names == expected, f"statefulset.name {direction} not sorted; got {names}"
