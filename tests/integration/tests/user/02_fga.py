...
def test_attach_detach_dual_scoped(
    ...
    admin_token = get_token(USER_ADMIN_EMAIL, USER_ADMIN_PASSWORD)
    role_id = find_role_id(admin_token, _ACTOR_ROLE_NAME)
...
def test_cleanup(
    ...
    resp = requests.get(signoz.self.host_configs["8080"].get(f"{USERS_BASE}/{actor['id']}/roles"), headers={"Authorization": f"Bearer {admin_token}"}, timeout=5)
    assert resp.status_code == HTTPStatus.OK, resp.text
    custom_entry = next((role for role in resp.json()["data"] if role["name"] == _ACTOR_ROLE_NAME), None)
    if custom_entry is not None:
        resp = requests.delete(
            signoz.self.host_configs["8080"].get(f"{USERS_BASE}/{actor['id']}/roles/{custom_entry['id']}"),
            headers={"Authorization": f"Bearer {admin_token}"},
            timeout=5,
        )
