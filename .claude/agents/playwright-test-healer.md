...
- **Auth.** `tests/e2e/fixtures/auth.ts` logs in once per worker and caches `storageState` (cookies + localStorage with `AUTH_TOKEN`). For API-driven seeding/cleanup, use `authToken(page)` from `utils/dashboards.ts` and pass `Authorization: Bearer <token>`. Never re-implement login.
