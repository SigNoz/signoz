...
if [[ -z "${SIGNOZ_API_KEY:-}" ]]; then
  ...
  echo "If the server runs against a reused local SQLite DB, extract an existing key instead:" >&2
  echo "  export SIGNOZ_API_KEY=\$(${BASH_SOURCE[0]%/*}/get-api-key.sh /path/to/signoz.db)" >&2
  exit 1
fi
