...
func (r *Runner) runCycle(ctx context.Context) {
	...
	nextTracker, event := r.tracker.Observe(report, now, r.AlertSeverity, r.FailuresBeforeAlert)
	if event == nil || r.Sink == nil {
		r.tracker = nextTracker
		return
	}
	if err := r.Sink.Notify(ctx, *event); err != nil {
		if r.OnError != nil {
			r.OnError(err)
		}
		return
...
func (t Tracker) Observe(report audit.Report, now time.Time, minimumSeverity string, failuresBeforeAlert int) (Tracker, *alerting.Event) {
	if minimumSeverity == "" {
