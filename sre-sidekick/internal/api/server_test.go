...
func TestAPIKeyProtectsNonHealthRoutes(t *testing.T) {
	server := httptest.NewServer(NewWithOptions(registry.New(), nil, Options{APIKey: "secret"}))
	defer server.Close()

	response, err := http.Get(server.URL + "/v1/profiles")
	if err != nil {
		t.Fatal(err)
	}
	response.Body.Close()
	if response.StatusCode != http.StatusUnauthorized {
		t.Fatalf("expected unauthorized response, got %s", response.Status)
	}
	...
	if response.StatusCode != http.StatusOK {
		t.Fatalf("expected authorized response, got %s", response.Status)
	}
	...
	response.Body.Close()
	if response.StatusCode != http.StatusOK {
		t.Fatalf("health endpoint must remain unauthenticated, got %s", response.Status)
	}
}

func TestRequestBodyLimit(t *testing.T) {
	handler := NewWithOptions(registry.New(), nil, Options{MaxBodyBytes: 32})
	request := httptest.NewRequest(
		http.MethodPost,
		"/v1/profiles",
		strings.NewReader(`{"padding":"`+strings.Repeat("x", 64)+`"}`),
	)
	...
	if response.Code != http.StatusRequestEntityTooLarge {
		t.Fatalf("expected request body limit, got %d: %s", response.Code, response.Body.String())
	}
}
