...
func New(ctx context.Context, settings factory.ProviderSettings, config web.Config) (web.Web, error) {
	...
	if err != nil {
		return nil, errors.WrapInternalf(err, errors.CodeInternal, "cannot access web directory")
	}

	ok := fi.IsDir()
	if !ok {
		return nil, errors.NewInternalf(errors.CodeInternal, "web directory is not a directory")
	}

	fi, err = os.Stat(filepath.Join(config.Directory, indexFileName))
	if err != nil {
		return nil, errors.WrapInternalf(err, errors.CodeInternal, "cannot access %q in web directory", indexFileName)
	}

	if os.IsNotExist(err) || fi.IsDir() {
		return nil, errors.NewInternalf(errors.CodeInternal, "%q does not exist", indexFileName)
	}
