...
func (m *fieldMapper) ExistsFor(_ context.Context, _ valuer.UUID, _, _ uint64, key *telemetrytypes.TelemetryFieldKey, exists bool) (string, error) {
	...
	if exists {
		return fmt.Sprintf("has(JSONExtractKeys(labels), %s)", clickhousesql.StringLiteral(key.Name)), nil
	}
	return fmt.Sprintf("not has(JSONExtractKeys(labels), %s)", clickhousesql.StringLiteral(key.Name)), nil
}
