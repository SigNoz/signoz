...
// intrinsic column is always present. A label has a key membership test.
// An absent label reads the empty string. That is the metrics keyless
// contract, so no query guards it. Every read keeps its native type. A
// label already reads back as String, and coercion adds nothing.
...
// is a label. A context-qualified name is also the label spelled with its
// context, because a context can be a legitimate prefix in user data.
