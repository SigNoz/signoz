...
func TestGetFieldKeyName(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			storage := NewStorage()
			read, err := storage.Read(ctx, qbtypes.QueryInfo{StartNs: uint64(time.Date(2024, 6, 1, 0, 0, 0, 0, time.UTC).UnixNano()), EndNs: uint64(time.Date(2024, 6, 5, 0, 0, 0, 0, time.UTC).UnixNano())}, &tc.key)
...
func TestColumnExpressionForTemporalColumn(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			requiredDataType: telemetrytypes.FieldDataTypeFloat64,
			expectedResult:   "toFloat64(duration_nano)",
		},
		{
			name:             "string intrinsic keeps its cast and takes no guard",
			key:              telemetrytypes.TelemetryFieldKey{Name: "name"},
			requiredDataType: telemetrytypes.FieldDataTypeFloat64,
			expectedResult:   "toFloat64OrNull(name)",
		},
		{
