	...
	cases := []struct {
		...
		},
		{
			...
			query: qbtypes.QueryBuilderQuery[qbtypes.TraceAggregation]{
				...
				Aggregations: []qbtypes.TraceAggregation{
					{
						Expression: "heatmap(duration_nano, 20)",
					},
				},
				Filter: &qbtypes.Filter{
					Expression: "service.name = 'redis-manual'",
			...
			expected: qbtypes.Statement{
				Query: "WITH __resource_filter AS (SELECT fingerprint FROM signoz_traces.distributed_traces_v3_resource WHERE (simpleJSONExtractString(labels, 'service.name') = ? AND labels LIKE ? AND labels LIKE ?) AND seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ?), __histogram_0 AS (SELECT histogram(20)(multiIf(duration_nano <> ?, duration_nano, NULL)) AS buckets_0 FROM signoz_traces.distributed_signoz_index_v3 WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ?), __buckets_0 AS (SELECT arrayJoin(arrayMap(i -> (i, buckets_0[i].1, buckets_0[i].2), range(1, length(buckets_0) + 1))) AS bucket_0 FROM __histogram_0), __counts_0 AS (SELECT toStartOfInterval(timestamp, INTERVAL 60 SECOND) AS ts, bucket_0.1 AS bucket_idx_0, count() AS cnt_0 FROM signoz_traces.distributed_signoz_index_v3 CROSS JOIN __buckets_0 WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ? AND duration_nano >= bucket_0.2 AND duration_nano < bucket_0.3 GROUP BY ts, bucket_idx_0), __timestamps AS (SELECT toStartOfInterval(timestamp, INTERVAL 60 SECOND) AS ts FROM signoz_traces.distributed_signoz_index_v3 WHERE true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ? GROUP BY ts) SELECT t.ts, arrayMap(x -> (x.2, x.3, x.4), arraySort(x -> x.1, groupArray((b_0.bucket_0.1, b_0.bucket_0.2, b_0.bucket_0.3, ifNull(c_0.cnt_0, 0))))) AS __result_0 FROM __timestamps t CROSS JOIN __buckets_0 b_0 LEFT JOIN __counts_0 c_0 ON c_0.ts = t.ts AND c_0.bucket_idx_0 = b_0.bucket_0.1 GROUP BY t.ts ORDER BY t.ts",
				Args:  []any{"redis-manual", "%service.name%", "%service.name\":\"redis-manual%", uint64(1747945619), uint64(1747983448), 0, "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448), "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448), "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448)},
			},
			expectedErr: nil,
		},
	}
