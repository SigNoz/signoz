...
func TestStatementBuilder(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query:       "WITH __resource_filter AS (SELECT fingerprint FROM signoz_traces.distributed_traces_v3_resource WHERE (simpleJSONExtractString(labels, 'service.name') = ? AND labels LIKE ? AND labels LIKE ?) AND seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ?) SELECT toStartOfInterval(timestamp, INTERVAL 60 SECOND) AS ts, histogram(20)(multiIf(duration_nano <> ?, duration_nano, NULL)) AS __result_0 FROM signoz_traces.distributed_signoz_index_v3 WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ? GROUP BY ts ORDER BY ts",
		...
		},
		{
			...
			query: qbtypes.QueryBuilderQuery[qbtypes.TraceAggregation]{
				...
				Aggregations: []qbtypes.TraceAggregation{
					{
						Expression: "distribution(duration_nano, 15)",
					},
				},
				Filter: &qbtypes.Filter{
			...
			expected: qbtypes.Statement{
				Query:       "WITH __resource_filter AS (SELECT fingerprint FROM signoz_traces.distributed_traces_v3_resource WHERE (simpleJSONExtractString(labels, 'service.name') = ? AND labels LIKE ? AND labels LIKE ?) AND seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ?) SELECT 1747947419000000000 AS ts, histogram(15)(multiIf(duration_nano <> ?, duration_nano, NULL)) AS __result_0 FROM signoz_traces.distributed_signoz_index_v3 WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ?",
				Args:        []any{"frontend", "%service.name%", "%service.name\":\"frontend%", uint64(1747945619), uint64(1747983448), 0, "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448)},
				BucketCount: 15,
			},
			expectedErr: nil,
		},
