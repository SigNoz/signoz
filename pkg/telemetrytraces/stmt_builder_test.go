...
func TestStatementBuilder(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "WITH __resource_filter AS (SELECT fingerprint FROM signoz_traces.distributed_traces_v3_resource WHERE (simpleJSONExtractString(labels, 'service.name') = ? AND labels LIKE ? AND labels LIKE ?) AND seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ?), __histogram_0 AS (SELECT histogram(20)(multiIf(duration_nano <> ?, duration_nano, NULL)) AS buckets_0 FROM signoz_traces.distributed_signoz_index_v3 WHERE true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ?) SELECT toStartOfInterval(timestamp, INTERVAL 60 SECOND) AS ts, arrayMap(i -> tuple(buckets_0[i].1, buckets_0[i].2, length(arrayFilter(d -> d >= buckets_0[i].1 AND d < buckets_0[i].2, groupArray(multiIf(duration_nano <> ?, duration_nano, NULL))))), range(1, length(buckets_0) + 1)) AS __result_0 FROM signoz_traces.distributed_signoz_index_v3 CROSS JOIN __histogram_0 WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ? GROUP BY ts, buckets_0 ORDER BY ts",
				Args:  []any{"redis-manual", "%service.name%", "%service.name\":\"redis-manual%", uint64(1747945619), uint64(1747983448), 0, "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448), 0, "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448)},
			},
			expectedErr: nil,
		},
	}
