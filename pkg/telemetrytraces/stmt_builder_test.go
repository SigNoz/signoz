...
func TestStatementBuilder(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expected: qbtypes.Statement{
				Query: "WITH __resource_filter AS (SELECT fingerprint FROM signoz_traces.distributed_traces_v3_resource WHERE (simpleJSONExtractString(labels, 'service.name') = ? AND labels LIKE ? AND labels LIKE ?) AND seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ?), global_bins_0 AS (SELECT min(duration_nano) AS global_min, max(duration_nano) AS global_max, (max(duration_nano) - min(duration_nano)) / 20 AS bin_width FROM signoz_traces.distributed_signoz_index_v3 WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ?), binned_data_0 AS (SELECT toStartOfInterval(timestamp, INTERVAL 60 SECOND) AS ts, floor((duration_nano - (SELECT global_min FROM global_bins_0)) / (SELECT bin_width FROM global_bins_0)) AS bin_index, (SELECT global_min FROM global_bins_0) + bin_index * (SELECT bin_width FROM global_bins_0) AS bin_upper FROM signoz_traces.distributed_signoz_index_v3 WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND true AND timestamp >= ? AND timestamp < ? AND ts_bucket_start >= ? AND ts_bucket_start <= ?), agg_data_0 AS (SELECT ts, groupArray([toFloat64(bin_upper) - (SELECT bin_width FROM global_bins_0), toFloat64(bin_upper), toFloat64(count)]) AS __result_0 FROM (SELECT ts, bin_upper, count() AS count FROM binned_data_0 WHERE bin_index >= 0 AND bin_index < 20 GROUP BY ts, bin_upper ORDER BY ts, bin_upper) GROUP BY ts) SELECT agg_data_0.ts AS ts, agg_data_0.__result_0 FROM agg_data_0 ORDER BY ts SETTINGS distributed_product_mode='allow', max_memory_usage=10000000000",
				Args:  []any{"redis-manual", "%service.name%", "%service.name\":\"redis-manual%", uint64(1747945619), uint64(1747983448), "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448), "1747947419000000000", "1747983448000000000", uint64(1747945619), uint64(1747983448)},
			},
			expectedErr: nil,
		},
	}
