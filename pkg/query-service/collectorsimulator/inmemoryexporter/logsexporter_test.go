...
func TestExporterLifecycle(t *testing.T) {
	...
	constructed, err := makeTestLogExporter(testExporterId)
	require.Nil(err, "could not make test exporter")
	err = constructed.Start(
		context.Background(),
	...
	constructed2, err := makeTestLogExporter(testExporterId)
	require.Nil(err, "could not create second exporter with same id")
	err = constructed2.Start(
		context.Background(),
		componenttest.NewNopHost(),
	)
	...
	constructed3, err := makeTestLogExporter(testExporterId)
	require.Nil(err, "could not make exporter with same Id after shutting down old one")
	err = constructed3.Start(
		context.Background(),
		componenttest.NewNopHost(),
	)
...
func makeTestLogExporter(exporterId string) (exporter.Logs, error) {
	factory := NewFactory()
