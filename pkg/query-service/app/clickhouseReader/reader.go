...
func (r *ClickHouseReader) SetUserPreferences(ctx context.Context, userPreferences *model.UserPreferences) *model.ApiError {
	...
	userPreferencesFound, apiError := r.GetUserPreferences(ctx)
	if apiError != nil {
		return apiError
	}

	if userPreferencesFound != nil {
		...
		if _, err := stmt.Exec(userPreferences.GetIsAnonymous(), userPreferences.GetHasOptedUpdate(), userPreferencesFound.GetId()); err != nil {
			zap.S().Errorf("Error in Executing prepared statement for INSERT to user_preferences\n", err)
			tx.Rollback() // return an error too, we may want to wrap them
			return &model.ApiError{Typ: model.ErrorInternal, Err: err}
		}
	} else {
		stmt, err := tx.Prepare(`INSERT INTO user_preferences (isAnonymous, hasOptedUpdates) VALUES($1,$2);`)
		defer stmt.Close()
		...
		if _, err := stmt.Exec(userPreferences.GetIsAnonymous(), userPreferences.GetHasOptedUpdate()); err != nil {
			zap.S().Errorf("Error in Executing prepared statement for INSERT to user_preferences\n", err)
			tx.Rollback() // return an error too, we may want to wrap them
			return &model.ApiError{Typ: model.ErrorInternal, Err: err}
		}
...
func (r *ClickHouseReader) GetUserPreferences(ctx context.Context) (*model.UserPreferences, *model.ApiError) {

	userPreferences := []model.UserPreferences{}

	query := fmt.Sprintf("SELECT id, isAnonymous, hasOptedUpdates FROM user_preferences")

	err := r.localDB.Select(&userPreferences, query)
