	...
	if err == nil {
		for idx := range res.Items {
			...
			if rule.AlertType == ruletypes.AlertTypeLogs {
				...
				res.Items[idx].RelatedLogsLink = contextlinks.PrepareLinksToLogsV5(start, end, whereClause).Encode()
				...
				whereClause := contextlinks.PrepareFilterExpression(lbls, filterExpr, q.GroupBy)
				res.Items[idx].RelatedTracesLink = contextlinks.PrepareLinksToTracesV5(start, end, whereClause).Encode()
			}
		}
	}
