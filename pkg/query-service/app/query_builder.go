...
type prepareLogsQueryFunc func(start, end, step int64, queryType v3.QueryType, panelType v3.PanelType, bq *v3.BuilderQuery, fields map[string]v3.AttributeKey) (string, error)
		...
		for queryName, query := range compositeQuery.BuilderQueries {
			if query.Expression == queryName {
				...
				case v3.DataSourceLogs:
					...
					queryString, err := qb.options.BuildLogQuery(params.Start, params.End, params.Step, compositeQuery.QueryType, compositeQuery.PanelType, query, fields)
					if err != nil {
						return nil, err
					}
					queries[queryName] = queryString
