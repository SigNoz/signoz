...
// getParamsForTopHosts returns the step, time series table name and samples table name for the top hosts query
// what are we doing here?
// we want to identify the top hosts quickly, so we use pre-aggregated data for samples and time series tables
// the speed of the query depends on the number of values in group by clause
// we also have fixed step sizes for different time ranges, because we are
// not interested in the exact values, or interval wise time series data
// so we use averaged values of high interval data
func getParamsForTopHosts(req model.HostListRequest) (int64, string, string) {
	...
	if req.End-req.Start < time.Hour.Milliseconds() {
	...
	} else if req.End-req.Start < time.Hour.Milliseconds()*6 {
		...
		samplesTableName = constants.SIGNOZ_SAMPLES_V4_AGG_5M_TABLENAME
	} else if req.End-req.Start < time.Hour.Milliseconds()*24 {
	...
	} else if req.End-req.Start < time.Hour.Milliseconds()*7 {
