...
type prepareTracesQueryFunc func(start, end int64, panelType v3.PanelType, bq *v3.BuilderQuery, options v3.QBOptions, filterBy string) (string, error)
				...
				case v3.DataSourceTraces:
					...
					if compositeQuery.PanelType == v3.PanelTypeGraph && query.Limit > 0 && len(query.GroupBy) > 0 {
						limitQuery, err := qb.options.BuildTraceQuery(start, end, compositeQuery.PanelType, query,
							v3.QBOptions{GraphLimitQtype: constants.FirstQueryGraphLimit, PreferRPM: PreferRPMFeatureEnabled}, params.FilterBy)
						if err != nil {
							return nil, err
						}
						placeholderQuery, err := qb.options.BuildTraceQuery(start, end, compositeQuery.PanelType,
							query, v3.QBOptions{GraphLimitQtype: constants.SecondQueryGraphLimit, PreferRPM: PreferRPMFeatureEnabled}, params.FilterBy)
						if err != nil {
							return nil, err
						}
						...
						queryString, err := qb.options.BuildTraceQuery(start, end, compositeQuery.PanelType,
							query, v3.QBOptions{PreferRPM: PreferRPMFeatureEnabled, GraphLimitQtype: ""}, params.FilterBy)
						if err != nil {
							return nil, err
						}
