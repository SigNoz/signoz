...
// SaveFunnel saves a funnel to the database in the trace_funnelss table
...
func (c *SQLClient) SaveFunnel(funnel *Funnel, userID, orgID string, tags, extraData string) error {
	...
	err = db.NewRaw("SELECT COUNT(*), IFNULL(created_by, ''), IFNULL(created_at, '') FROM trace_funnelss WHERE uuid = ? AND category = 'funnel'", funnel.ID).
	...
	if count > 0 {
		...
		_, err = db.NewRaw(
			"UPDATE trace_funnelss SET name = ?, data = ?, updated_by = ?, updated_at = ?, tags = ?, extra_data = ? WHERE uuid = ? AND category = 'funnel'",
...
func (c *SQLClient) CheckFunnelNameCollision(name, userID string) (bool, error) {
	...
	var count int
	err := db.NewRaw("SELECT COUNT(*) FROM trace_funnelss WHERE name = ? AND created_by = ? AND category = 'funnel'", name, userID).
		Scan(ctx, &count)
...
func (c *SQLClient) CreateFunnel(funnel *Funnel, userID, orgID string) error {
	...
	// Insert new funnel
	_, err = db.NewRaw(
		"INSERT INTO trace_funnelss (uuid, name, category, created_by, updated_by, source_page, data, created_at, updated_at, org_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
func (c *SQLClient) UpdateFunnel(funnel *Funnel, userID string) error {
	...
	_, err = db.NewRaw(
		"UPDATE trace_funnelss SET data = ?, updated_by = ?, updated_at = ? WHERE uuid = ? AND category = 'funnel'",
...
func (c *SQLClient) GetFunnelMetadata(funnelID string) (string, string, string, string, error) {
	...
	err := db.NewRaw(
		"SELECT created_at, updated_at, IFNULL(tags, ''), IFNULL(extra_data, '') FROM trace_funnelss WHERE uuid = ? AND category = 'funnel'",
		funnelID,
...
func (c *SQLClient) GetFunnelExtraData(funnelID string) (string, string, error) {
	...
	err := db.NewRaw(
		"SELECT IFNULL(extra_data, ''), IFNULL(tags, '') FROM trace_funnelss WHERE uuid = ? AND category = 'funnel'",
		funnelID,
