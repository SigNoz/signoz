...
// filter condition, and the column expression, written once. A storage answers
...
const (
	...
	// the exists guard on positive operators and in every select field, group by,
	// order by, and aggregation, and
...
type Read struct {
	...
	// legacy string body). A select field, group by, order by, or aggregation
	// drops such a
...
// condition language overrides. Resolution, the filter condition, and the column
// expression live in querybuilder and are written once.
