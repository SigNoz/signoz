...
func TestValidateFieldKeyText(t *testing.T) {
	tests := []struct {
		name      string
		key       string
		expectErr bool
		...
		{
			name:      "valid field with context",
			key:       "http.status_code:float",
			expectErr: false,
		},
		{
			name:      "invalid field with context",
			key:       "http.status_code:float:extra",
			expectErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidateFieldKeyText(tt.key)
