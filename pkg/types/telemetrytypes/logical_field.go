...
type LogicalField struct {
	...
	// ValueMaps is index-aligned with Members. Each entry is the map that
	// brings a member's stored values into the current member's vocabulary.
	// It is nil when the vocabularies are the same. Resolution sets it on
	// this wrapper, and the reads apply it.
	ValueMaps []*ValueMap
}
