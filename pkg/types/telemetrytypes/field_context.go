...
// IsContextPrefixAllowedForSignal returns whether the reserved context prefix (e.g. `resource.`)
// should be interpreted as a FieldContext for the given signal.
func IsContextPrefixAllowedForSignal(signal Signal, ctx FieldContext) bool {
	if signal == SignalUnspecified {
		return true
	}
