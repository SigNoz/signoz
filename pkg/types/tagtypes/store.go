...
type Store interface {
	...
	// On conflict on (org_id, LOWER(key), LOWER(value)) — which happens only
	// when a concurrent insert raced ours, including casing-only collisions —
	// the returned entry carries the existing row's ID rather than the
	// pre-generated one in the input.
