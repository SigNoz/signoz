...
func TestPromQLFilterExtractor_Extract(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantGroupBy: []string{}, // without() means no grouping keys per spec
		},
		{
		...
		},
		{
			...
			wantMetrics: []string{"cpu_usage_total"},
			wantGroupBy: []string{"region"}, // Only outermost grouping
		},
		{
			name:        "P7a - Nested aggregation: inner grouping ignored",
			query:       `sum(max by (pod) (cpu_usage_total{env="prod"}))`,
			wantMetrics: []string{"cpu_usage_total"},
			wantGroupBy: []string{}, // Inner grouping is ignored when outer has no grouping (nestingLevel != 0 case)
		},
		{
