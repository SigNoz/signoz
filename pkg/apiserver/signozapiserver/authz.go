...
func (provider *provider) addAuthzRoutes(router *mux.Router) error {
	...
	if err := router.Handle("/api/v1/roles/{id}/relation/{relation}", handler.New(provider.authZ.AdminAccess(provider.authzHandler.GetObjects), handler.OpenAPIDef{
		ID:                  "GetObjects",
		Tags:                []string{"role"},
	...
	if err := router.Handle("/api/v1/roles/{id}/relation/{relation}", handler.New(provider.authZ.AdminAccess(provider.authzHandler.PatchObjects), handler.OpenAPIDef{
		ID:                  "PatchObjects",
		Tags:                []string{"role"},
