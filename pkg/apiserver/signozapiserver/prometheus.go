...
// prometheusOpenAPIHandler documents a Prometheus-compatible endpoint. The
// default handler wrapper is not used because it wraps every response in
// the house envelope; these endpoints follow Prometheus' wire contract,
// described by the prometheus package's *Schema types. The contract's
// authority is upstream (https://prometheus.io/docs/prometheus/latest/querying/api/);
// the operation only documents it.
