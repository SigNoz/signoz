...
func (provider *provider) addAlertmanagerRoutes(router *mux.Router) error {
	...
	if err := router.Handle("/api/v2/notification_channels/{id}", handler.New(
		...
		handler.OpenAPIDef{
			...
			Tags:                []string{"channels"},
			Summary:             "Get notification channel by ID",
			Description:         "This endpoint returns a notification channel by ID. A channel written by the v1 API can carry a configuration this API does not model, which is reported as unprocessable.",
			Request:             nil,
			RequestContentType:  "",
			Response:            new(alertmanagertypes.GettableNotificationChannel),
			ResponseContentType: "application/json",
			SuccessStatusCode:   http.StatusOK,
			ErrorStatusCodes:    []int{http.StatusNotFound, http.StatusUnprocessableEntity},
			Deprecated:          false,
