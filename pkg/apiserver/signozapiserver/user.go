...
func (provider *provider) addUserRoutes(router *mux.Router) error {
	...
	if err := router.Handle("/api/v2/users/{id}/roles", handler.New(
		provider.authzMiddleware.CheckResources(provider.userHandler.SetRoleByUserID, authtypes.SigNozAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "SetRoleByUserID",
			Tags:                []string{"users"},
		...
		handler.WithResourceDefs(handler.AttachDetachSiblingResourceDef{
			...
			TargetIDs:      coretypes.OneID(coretypes.BodyJSONPath("name")),
			TargetSelector: provider.roleNameSelector,
		}),
	)).Methods(http.MethodPost).GetError(); err != nil {
		return err
	}
	...
	if err := router.Handle("/api/v2/users/{id}/roles/{roleId}", handler.New(
		provider.authzMiddleware.CheckResources(provider.userHandler.RemoveUserRoleByRoleID, authtypes.SigNozAdminRoleName),
		handler.OpenAPIDef{
		...
		handler.WithResourceDefs(handler.AttachDetachSiblingResourceDef{
			...
			SourceSelector: coretypes.IDSelector,
			TargetResource: coretypes.ResourceRole,
			TargetIDs:      coretypes.OneID(coretypes.PathParam("roleId")),
...
func (provider *provider) roleNameSelector(_ context.Context, resource coretypes.Resource, id string, _ valuer.UUID) ([]coretypes.Selector, error) {
