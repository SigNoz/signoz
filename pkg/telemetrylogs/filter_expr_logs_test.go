...
func TestFilterExprLogs(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (match(LOWER(body), LOWER(?)) AND ('resource.service.name' = ? AND 'resource.service.name' <> ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ('resource.service.name' = ? AND 'resource.service.name' <> ?)",
		...
		},
		{
			...
			query:                 "service.name!=\"api\"",
			shouldPass:            true,
			expectedQuery:         "WHERE 'resource.service.name' <> ?",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (('resource.service.name' = ? OR 'resource.service.name' = ? OR 'resource.service.name' = ?) AND 'resource.service.name' <> ?)",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (('resource.environment' = ? OR 'resource.environment' = ? OR 'resource.environment' = ? OR 'resource.environment' = ?) AND 'resource.environment' <> ?)",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ('resource.environment' <> ?)",
		...
		// Explicit AND
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?) AND ('resource.service.name' = ? AND 'resource.service.name' <> ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (('resource.service.name' = ? AND 'resource.service.name' <> ?) OR ('resource.service.name' = ? AND 'resource.service.name' <> ?))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE NOT (('resource.service.name' = ? AND 'resource.service.name' <> ?))",
			expectedArgs:          []any{"api", "NULL"},
			expectedErrorContains: "",
		},
		{
		...
		// AND + OR combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?) AND ((('resource.service.name' = ? AND 'resource.service.name' <> ?) OR ('resource.service.name' = ? AND 'resource.service.name' <> ?))))",
		...
		// AND + NOT combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?) AND NOT (('resource.service.name' = ? AND 'resource.service.name' <> ?)))",
			expectedArgs:          []any{float64(200), true, "api", "NULL"},
			expectedErrorContains: "",
		},
		{
		...
		// OR + NOT combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (NOT ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?)) OR NOT (('resource.service.name' = ? AND 'resource.service.name' <> ?)))",
			expectedArgs:          []any{float64(200), true, "api", "NULL"},
			expectedErrorContains: "",
		},
		{
		...
		// AND + OR + NOT combinations
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?) AND ((('resource.service.name' = ? AND 'resource.service.name' <> ?) OR NOT ((toFloat64(attributes_number['duration']) > ? AND mapContains(attributes_number, 'duration') = ?)))))",
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (NOT ((((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?) AND ('resource.service.name' = ? AND 'resource.service.name' <> ?)))) OR (toFloat64(attributes_number['count']) > ? AND mapContains(attributes_number, 'count') = ?))",
		...
		// Mixed implicit and explicit AND
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?) AND ('resource.service.name' = ? AND 'resource.service.name' <> ?) AND (toFloat64(attributes_number['duration']) < ? AND mapContains(attributes_number, 'duration') = ?))",
			expectedArgs:          []any{float64(200), true, "api", "NULL", float64(1000), true},
			expectedErrorContains: "",
		},
		{
		...
		},
		{
			...
			shouldPass:            true,
			expectedQuery:         "WHERE (((toFloat64(attributes_number['count']) > ? AND mapContains(attributes_number, 'count') = ?) AND (((((toFloat64(attributes_number['duration']) < ? AND mapContains(attributes_number, 'duration') = ?) AND ('resource.service.name' = ? AND 'resource.service.name' <> ?))) OR (((toFloat64(attributes_number['duration']) < ? AND mapContains(attributes_number, 'duration') = ?) AND ('resource.service.name' = ? AND 'resource.service.name' <> ?)))))))",
		...
		// Testing NOT > AND > OR precedence
		{
			...
			shouldPass:    true,
			expectedQuery: "WHERE (NOT ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?)) AND ('resource.service.name' = ? AND 'resource.service.name' <> ?))",
		...
		},
		{
			...
			shouldPass:    true,
			expectedQuery: "WHERE ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?) OR (('resource.service.name' = ? AND 'resource.service.name' <> ?) AND (attributes_string['level'] = ? AND mapContains(attributes_string, 'level') = ?)))",
		...
		`,
			shouldPass:    true,
			expectedQuery: "WHERE ((((((((toFloat64(attributes_number['status']) >= ? AND mapContains(attributes_number, 'status') = ?) AND (toFloat64(attributes_number['status']) < ? AND mapContains(attributes_number, 'status') = ?))) OR (((toFloat64(attributes_number['status']) >= ? AND mapContains(attributes_number, 'status') = ?) AND (toFloat64(attributes_number['status']) < ? AND mapContains(attributes_number, 'status') = ?) AND NOT ((toFloat64(attributes_number['status']) = ? AND mapContains(attributes_number, 'status') = ?)))))) AND (((('resource.service.name' = ? OR 'resource.service.name' = ? OR 'resource.service.name' = ?) AND 'resource.service.name' <> ?) OR ((('resource.service.type' = ? AND 'resource.service.type' <> ?) AND NOT (('resource.service.deprecated' = ? AND 'resource.service.deprecated' <> ?)))))))) AND (((((toFloat64(attributes_number['duration']) < ? AND mapContains(attributes_number, 'duration') = ?) OR ((toFloat64(attributes_number['duration']) BETWEEN ? AND ? AND mapContains(attributes_number, 'duration') = ?)))) AND (('resource.environment' <> ? OR ((('resource.environment' = ? AND 'resource.environment' <> ?) AND (attributes_bool['is_automated_test'] = ? AND mapContains(attributes_bool, 'is_automated_test') = ?))))))) AND NOT ((((((LOWER(attributes_string['message']) LIKE LOWER(?) AND mapContains(attributes_string, 'message') = ?) OR (LOWER(attributes_string['message']) LIKE LOWER(?) AND mapContains(attributes_string, 'message') = ?))) AND (attributes_string['severity'] = ? AND mapContains(attributes_string, 'severity') = ?)))))",
			expectedArgs: []any{
