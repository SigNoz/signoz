...
func (c *conditionBuilder) conditionFor(
	...
	case qbtypes.FilterOperatorExists, qbtypes.FilterOperatorNotExists:
		...
		if len(key.Evolutions) > 0 {
			...
			if len(newColumns) == 0 {
				return "", errors.NewInvalidInputf(errors.CodeInvalidInput, "no valid evolution found for field %s in the given time range", key.Name).WithInvalidReferences(key.String())
			}
