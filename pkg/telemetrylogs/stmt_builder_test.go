...
func TestStatementBuilderTimeSeriesBodyGroupBy(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expected: qbtypes.Statement{
				Query: "WITH __resource_filter AS (SELECT fingerprint FROM signoz_logs.distributed_logs_v2_resource WHERE seen_at_ts_bucket_start >= ? AND seen_at_ts_bucket_start <= ?), __limit_cte AS (SELECT toString(dynamicElement(_x_education.awards.name, 'String')) AS `body.education:awards.name`, toString(dynamicElement(_x_positions.ratings, 'Array(Nullable(Int64))')) AS `body.interests:entities:reviews:entries:metadata:positions:ratings`, count() AS __result_0 FROM signoz_logs.distributed_logs_v2 ARRAY JOIN dynamicElement(body_v2.education, 'Array(JSON(max_dynamic_types=16, max_dynamic_paths=0))') AS _x_education ARRAY JOIN dynamicElement(body_v2.interests, 'Array(JSON(max_dynamic_types=16, max_dynamic_paths=0))') AS _x_interests ARRAY JOIN dynamicElement(_x_interests.entities, 'Array(JSON(max_dynamic_types=8, max_dynamic_paths=0))') AS _x_entities ARRAY JOIN dynamicElement(_x_entities.reviews, 'Array(JSON(max_dynamic_types=4, max_dynamic_paths=0))') AS _x_reviews ARRAY JOIN dynamicElement(_x_reviews.entries, 'Array(JSON(max_dynamic_types=2, max_dynamic_paths=0))') AS _x_entries ARRAY JOIN dynamicElement(_x_entries.metadata, 'Array(JSON(max_dynamic_types=1, max_dynamic_paths=0))') AS _x_metadata ARRAY JOIN dynamicElement(_x_metadata.positions, 'Array(JSON(max_dynamic_types=0, max_dynamic_paths=0))') AS _x_positions WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND timestamp >= ? AND ts_bucket_start >= ? AND timestamp < ? AND ts_bucket_start <= ? GROUP BY `body.education:awards.name`, `body.interests:entities:reviews:entries:metadata:positions:ratings` ORDER BY __result_0 DESC LIMIT ?) SELECT toStartOfInterval(fromUnixTimestamp64Nano(timestamp), INTERVAL 30 SECOND) AS ts, toString(dynamicElement(_x_education.awards.name, 'String')) AS `body.education:awards.name`, toString(dynamicElement(_x_positions.ratings, 'Array(Nullable(Int64))')) AS `body.interests:entities:reviews:entries:metadata:positions:ratings`, count() AS __result_0 FROM signoz_logs.distributed_logs_v2 ARRAY JOIN dynamicElement(body_v2.education, 'Array(JSON(max_dynamic_types=16, max_dynamic_paths=0))') AS _x_education ARRAY JOIN dynamicElement(body_v2.interests, 'Array(JSON(max_dynamic_types=16, max_dynamic_paths=0))') AS _x_interests ARRAY JOIN dynamicElement(_x_interests.entities, 'Array(JSON(max_dynamic_types=8, max_dynamic_paths=0))') AS _x_entities ARRAY JOIN dynamicElement(_x_entities.reviews, 'Array(JSON(max_dynamic_types=4, max_dynamic_paths=0))') AS _x_reviews ARRAY JOIN dynamicElement(_x_reviews.entries, 'Array(JSON(max_dynamic_types=2, max_dynamic_paths=0))') AS _x_entries ARRAY JOIN dynamicElement(_x_entries.metadata, 'Array(JSON(max_dynamic_types=1, max_dynamic_paths=0))') AS _x_metadata ARRAY JOIN dynamicElement(_x_metadata.positions, 'Array(JSON(max_dynamic_types=0, max_dynamic_paths=0))') AS _x_positions WHERE resource_fingerprint GLOBAL IN (SELECT fingerprint FROM __resource_filter) AND timestamp >= ? AND ts_bucket_start >= ? AND timestamp < ? AND ts_bucket_start <= ? AND (`body.education:awards.name`, `body.interests:entities:reviews:entries:metadata:positions:ratings`) GLOBAL IN (SELECT `body.education:awards.name`, `body.interests:entities:reviews:entries:metadata:positions:ratings` FROM __limit_cte) GROUP BY ts, `body.education:awards.name`, `body.interests:entities:reviews:entries:metadata:positions:ratings`",
