...
func (b *StatementBuilder) familyMetricNames(ctx context.Context, orgID valuer.UUID, metricName string) []string {
	return querybuilder.FamilyMetricNames(ctx, orgID, b.flagger, metricName)
}
...
func (b *StatementBuilder) buildReducedTimeSeriesCTE(
	...
	for i, g := range query.GroupBy {
		col, err := b.fm.ColumnExpressionFor(ctx, orgID, start, end, &g.TelemetryFieldKey, querybuilder.MatchingLogicalFields(ctx, orgID, b.flagger, telemetrytypes.SignalMetrics, &telemetrytypes.MetricContext{MetricName: query.Aggregations[0].MetricName}, &g.TelemetryFieldKey, keys), telemetrytypes.FieldDataTypeString, keys)
		if err != nil {
			return "", nil, err
		}
