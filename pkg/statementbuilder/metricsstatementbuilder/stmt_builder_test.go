...
func TestStatementBuilder(t *testing.T) {
	cases := []struct {
		...
		{
			// A user-supplied `le` is dropped from the final select, so it is grouped last:
			// the keys around it keep the positions their CTE aliases were built from, and
			// the statement matches test_histogram_percentile1 byte for byte.
