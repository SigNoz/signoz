...
type openfgaLogger struct {
	slog      *slog.Logger
	converter instrumentation.ZapToSlogConverter
}

func NewLogger(logger *slog.Logger) *openfgaLogger {
	return &openfgaLogger{slog: logger, converter: instrumentation.NewZapToSlogConverter()}
}

func (logger *openfgaLogger) With(fields ...zap.Field) pkgopenfgalogger.Logger {
	newLogger := logger.slog.With(logger.converter.FieldsToAttributes(fields)...)
	return &openfgaLogger{slog: newLogger, converter: logger.converter}
}

func (logger *openfgaLogger) Info(message string, fields ...zap.Field) {
	logger.slog.Info(message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}

func (logger *openfgaLogger) InfoWithContext(ctx context.Context, message string, fields ...zap.Field) {
	logger.slog.InfoContext(ctx, message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}

func (logger *openfgaLogger) Debug(message string, fields ...zap.Field) {
	logger.slog.Debug(message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}

func (logger *openfgaLogger) DebugWithContext(ctx context.Context, message string, fields ...zap.Field) {
	logger.slog.DebugContext(ctx, message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}

func (logger *openfgaLogger) Warn(message string, fields ...zap.Field) {
	logger.slog.Warn(message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}

func (logger *openfgaLogger) WarnWithContext(ctx context.Context, message string, fields ...zap.Field) {
	logger.slog.WarnContext(ctx, message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}

func (logger *openfgaLogger) Error(message string, fields ...zap.Field) {
	logger.slog.Error(message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}

func (logger *openfgaLogger) ErrorWithContext(ctx context.Context, message string, fields ...zap.Field) {
	logger.slog.ErrorContext(ctx, message, logger.converter.FieldsToAttributes(fields)...) //nolint:sloglint
}
