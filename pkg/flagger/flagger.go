...
func (f *flagger) Boolean(ctx context.Context, flag featuretypes.Name, evalCtx featuretypes.FlaggerEvaluationContext) (bool, error) {
	...
	if err != nil {
		f.settings.Logger().ErrorContext(ctx, "failed to get feature from default registry", errors.Attr(err), "flag", flag)
		return false, err
	}
	...
	if err != nil {
		...
		f.settings.Logger().ErrorContext(ctx, "failed to get default value from feature", errors.Attr(err), "flag", flag)
		return false, err
	}
	...
	for _, client := range f.clients {
		...
		if err != nil {
			f.settings.Logger().ErrorContext(ctx, "failed to get value from client", errors.Attr(err), "flag", flag, "client", client.Metadata().Name)
			continue
		}
...
func (f *flagger) String(ctx context.Context, flag featuretypes.Name, evalCtx featuretypes.FlaggerEvaluationContext) (string, error) {
	...
	for _, client := range f.clients {
		...
		if err != nil {
			f.settings.Logger().WarnContext(ctx, "failed to get value from client", errors.Attr(err), "flag", flag, "client", client.Metadata().Name)
			continue
		}
...
func (f *flagger) BooleanOrEmpty(ctx context.Context, flag featuretypes.Name, evalCtx featuretypes.FlaggerEvaluationContext) bool {
	...
	if err != nil {
		f.settings.Logger().ErrorContext(ctx, "failed to get value from flagger service", errors.Attr(err), "flag", flag)
		return defaultValue
	}
	return value
}
...
func (f *flagger) List(ctx context.Context, evalCtx featuretypes.FlaggerEvaluationContext) ([]*featuretypes.GettableFeature, error) {
	...
	for _, provider := range f.providers {
		...
		if err != nil {
			f.settings.Logger().WarnContext(ctx, "failed to get features from provider", errors.Attr(err), "provider", provider.Metadata().Name)
			continue
		}
