...
func TestServerSetConfigAndStop(t *testing.T) {
	...
	server, err := New(context.Background(), slog.New(slog.DiscardHandler), prometheus.NewRegistry(), NewConfig(), "1", alertmanagertypestest.NewStateStore(), notificationManager, newTestMaintenanceStore(), alertmanagernotify.NewReceiverIntegrationsFactory())
...
func TestServerPutAlerts(t *testing.T) {
	...
	notificationManager := nfmanagertest.NewMock()
	server, err := New(context.Background(), slog.New(slog.DiscardHandler), prometheus.NewRegistry(), srvCfg, "1", stateStore, notificationManager, newTestMaintenanceStore(), alertmanagernotify.NewReceiverIntegrationsFactory())
	require.NoError(t, err)
