...
func TestEndToEndAlertManagerFlow(t *testing.T) {
	...
	server, err := New(context.Background(), logger, registry, srvCfg, orgID, stateStore, notificationManager, maintenanceStore, alertmanagernotify.NewReceiverIntegrationsFactory())
	require.NoError(t, err)
