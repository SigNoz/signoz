	...
	for i := 0; i < len(s); i++ {
		...
		case parseAllColumnsState_Beginning:
			...
			if s[i] == '[' {
				...
				return columns, errors.NewInvalidInputf(errors.CodeInvalidInput, "unexpected token: %s", string(s[i]))
			}
		...
		case parseAllColumnsState_ReadingRawName:
			...
			if isQuote(s[i]) {
				return nil, errors.NewInvalidInputf(errors.CodeInvalidInput, "unexpected token: %s", string(s[i]))
			}
			if isSpace(s[i]) {
