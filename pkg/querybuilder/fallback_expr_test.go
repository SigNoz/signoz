...
func TestDataTypeCollisionHandledFieldName(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedFieldName: "toFloat64OrNull(toString(attribute_string_http$$status_code))",
		...
		},
		{
			...
			expectedFieldName: "toFloat64OrNull(toString(attribute_string_response$$times))",
		...
		},
		{
			...
			value:             "9",
			tblFieldName:      "attribute_float64_http$$request$$duration",
			expectedFieldName: "toFloat64OrNull(toString(attribute_float64_http$$request$$duration))",
			expectedValue:     "9",
