...
func PrepareWhereClause(query string, opts FilterExprVisitorOpts, startNs uint64, endNs uint64) (*PreparedWhereClause, error) {
	...
	if cond == "" {
		cond = SkipConditionLiteral
	}

	// Replace all SkipConditionLiteral with TrueConditionLiteral
	cond = strings.ReplaceAll(cond, SkipConditionLiteral, TrueConditionLiteral)
...
func (v *filterExpressionVisitor) VisitUnaryExpression(ctx *grammar.UnaryExpressionContext) any {
	...
	if ctx.NOT() != nil {
		...
		if result == SkipConditionLiteral {
			return result
		}
