...
func NewModule(ts telemetrystore.TelemetryStore, telemetryMetadataStore telemetrytypes.MetadataStore, cache cache.Cache, dashboardModule dashboard.Module, providerSettings factory.ProviderSettings) metricsmodule.Module {
...
func (m *module) GetMetricDashboards(ctx context.Context, orgID valuer.UUID, metricName string) (*metricsmoduletypes.MetricDashboardsResponse, error) {
	if metricName == "" {
	...
	dashboards := make([]metricsmoduletypes.MetricDashboard, 0)
	if dashboardList, ok := data[metricName]; ok {
		dashboards = make([]metricsmoduletypes.MetricDashboard, 0, len(dashboardList))
		for _, item := range dashboardList {
			dashboards = append(dashboards, metricsmoduletypes.MetricDashboard{
	...
	return &metricsmoduletypes.MetricDashboardsResponse{
