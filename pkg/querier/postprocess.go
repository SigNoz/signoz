...
func (q *querier) applyFormulas(results map[string]*qbtypes.Result, req *qbtypes.QueryRangeRequest) map[string]*qbtypes.Result {
	...
	for name, formula := range formulaQueries {
		...
		if req.RequestType == qbtypes.RequestTypeTimeSeries {
			result := q.processTimeSeriesFormula(results, formula, req)
			if result != nil {
				results[name] = result
			}
		}
...
func (q *querier) processTimeSeriesFormula(
	...
	if err != nil {
		q.logger.Error("failed to create formula evaluator", "error", err, "formula", formula.Name)
		return nil
	}
	...
	if err != nil {
		q.logger.Error("failed to evaluate formula", "error", err, "formula", formula.Name)
		return nil
	}
