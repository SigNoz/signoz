...
func (migration *migrateRbacToAuthz) Up(ctx context.Context, db *bun.DB) error {
	...
	for _, orgID := range orgIDs {
		...
		rows, err := tx.QueryContext(ctx, `
			SELECT id, name FROM role WHERE org_id = ?`, orgID)
		if err != nil {
			return err
		}
		defer rows.Close()
	...
	for _, tuple := range tuples {
		...
		if migration.sqlstore.BunDB().Dialect().Name() == dialect.PG {
			...
			ON CONFLICT (store, object_type, object_id, relation, _user) DO NOTHING`,
				storeID, "role", "organization/"+tuple.OrgID+"/role/"+tuple.RoleID, "assignee", "user:organization/"+tuple.OrgID+"/user/"+tuple.UserID, "user", tupleID, now,
			)
			if err != nil {
				return err
			}
			...
			ON CONFLICT (store, ulid, object_type) DO NOTHING`,
				storeID, "role", "organization/"+tuple.OrgID+"/role/"+tuple.RoleID, "assignee", "user:organization/"+tuple.OrgID+"/user/"+tuple.UserID, "TUPLE_OPERATION_WRITE", tupleID, now,
			)
			if err != nil {
				return err
			}
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			ON CONFLICT (store, ulid, object_type) DO NOTHING`,
				storeID, "role", "organization/"+tuple.OrgID+"/role/"+tuple.RoleID, "assignee", "user", "organization/"+tuple.OrgID+"/user/"+tuple.UserID, "", 0, tupleID, now,
			)
			if err != nil {
				return err
			}
