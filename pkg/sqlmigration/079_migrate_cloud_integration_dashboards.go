...
//go:embed 079_cloud_integration_dashboards
...
func (m *migrateCloudIntegrationDashboards) Up(ctx context.Context, db *bun.DB) error {
	...
	for _, os := range toProvision {
		...
		for dashName, dashJSON := range serviceDashes {
			source := fmt.Sprintf("cloud-integration--%s--%s--%s", os.provider, os.serviceID, dashName)
...
func loadCIDashboardDefs() (map[string]map[string]map[string]json.RawMessage, error) {
	...
	err := fs.WalkDir(ciDashboardFiles, "079_cloud_integration_dashboards", func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		...
		// path: 079_cloud_integration_dashboards/{provider}/{service}/{file}.json
		rel := strings.TrimPrefix(path, "079_cloud_integration_dashboards/")
